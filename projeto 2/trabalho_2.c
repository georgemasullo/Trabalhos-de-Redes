...
typedef struct
{
  struct pkt ultimoPacoteEnviado;
  int seqNum;
} informacao;
...
//Função de geração de número de checksum
int checksum(struct pkt *packet)
{
  ...
  for (int i = 0; i < 20; ++i)
    checksum = checksum + packet->payload[i]; //Soma do checksum aos bytes de dados (como sugerido no trabalho)
  return checksum;
}
...
void A_output(struct msg message)
{
  ...
  //Move dados para o pacote
  for (int i = 0; i < 20; i++)
  {
    packet.payload[i] = message.data[i];
  }
  packet.checksum = checksum(&packet);
  //Informação de ultimo pacote enviado
  A.ultimoPacoteEnviado = packet;
  printf("A Enviando\n");
  printf("A seq num %d\n", A.seqNum);
  tolayer3(0, packet);  //pacote sera enviado de A para B pela rede
  starttimer(0, TEMPO); //inicia o temporizador de A com valor estimado
  esperandoACK = SIM;
}

void B_output(struct msg message;) // completada para ponto extra
{
}
...
void A_input(struct pkt packet)
{
  ...
  {
    printf(" A:  Ferrou Ficou Corrompido\n");
    return;
  }
  if (packet.acknum > A.seqNum)
  {
    printf("  A: nao recebeu o certo :( \n");
    printf("A seq num %d\n", A.seqNum);
    printf("pkt ack num %d\n", packet.acknum);
    return;
  }
  stoptimer(0); //Função que para o temporizador
  printf("A Recebeu ACK\n");
  printf("A seq num %d\n", A.seqNum);
  A.seqNum++;
  esperandoACK = NAO;
}
...
void A_timerinterrupt()
{
  ...
  printf("A Reenviando\n");
  tolayer3(0, A.ultimoPacoteEnviado); //Envia de A para B pela rede
  starttimer(0, TEMPO);
}
...
void B_input(struct pkt packet)
{
  if (packet.checksum != checksum(&packet))
  {
    printf(" B:  Ferrou Ficou Corrompido :(\n");
    enviarACK(1, B.seqNum);
    return;
  }
  if (packet.seqnum == B.seqNum - 1)
  {
    printf("B: ja tinha enviado ACK, reenviando :/  \n");
    enviarACK(1, packet.seqnum);
    return;
  }
  if (packet.seqnum != B.seqNum)
  {
    printf("B: nao recebeu o certo :(  \n");
